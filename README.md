...
定时频道投递会在发送前将帖子信息保存到 `data/processed_posts.json`。
其中 `posts` 保留最近的 200 个已处理 ID，`pending_posts` 按顺序保存所有待发送帖的
`id`、`title`、`author`、`url`，不受 200 条限制；旧缓存无需手动迁移。

启动后会恢复队列，并按现有检查间隔重新抓取、尝试发送，即使帖子已离开最新列表。
队首失败会保留队列等待下一轮，成功才移出队列；论坛提示消息遵循相同规则。
手动私聊请求不入队，消息正文不会缓存，也不设置最大重试次数。

状态使用临时文件刷新落盘后原子替换，写入失败会报告错误并停止继续投递。
若 Telegram 已接收消息、但成功状态保存前进程崩溃，重启后可能重复发送。
